...
// 	p1 = fork();
// 	if (p1 < 0)
// 		ft_perror_exit(data, NULL,NULL, 127);
// 	if (p1 == 0)
// 	{
// 		close(outfile_fd);
...
static pid_t	ft_first_child(t_data *data, int pipe_fd[],int infile_fd)
{
	...
	if (p1 == 0)
	{
		...
		{
			close(pipe_fd[0]);
			if (!ft_get_path(data->envp))
        		ft_perror_exit(data, NULL, NULL, 0);
			ft_handle_execve_error(data, NULL, data->commands[0][0], ENOENT);
		}
		else if(execve(data->cmd_paths[0], data->commands[0], data->envp) == -1)
			ft_handle_execve_error(data, NULL, data->commands[0][0], errno);
	}
	return (p1);
}
...
static pid_t	ft_second_child(t_data *data, int pipe_fd[],int outfile_fd)
{
	...
	{
		close(pipe_fd[1]);
		if (dup2(pipe_fd[0], STDIN_FILENO) < 0 || dup2(outfile_fd, STDOUT_FILENO) < 0)
			ft_perror_exit(data, NULL, "dup (2)", 1);
		close(outfile_fd);
		close(pipe_fd[0]);
		if (data->cmd_paths[1] == NULL)
		...
		else if (execve(data->cmd_paths[1], data->commands[1], data->envp) == -1)
			ft_handle_execve_error(data, NULL, data->commands[1][0], errno);
	}
	return (p2);
}
...
// 	pipe(pipe_fd);
// 	p1 = ft_first_child(data, pipe_fd, infile_fd, outfile_fd);
// 	waitpid(p1, &status, 0);
// 	exit_code_p1 = (status >> 8) & 0xFF;
// 	close(pipe_fd[1]);
// 	if (exit_code_p1 != 0)
// 	{
// 		close(pipe_fd[0]);
// 		return (exit_code_p1);
// 	}
// 	p2 = ft_second_child(data, pipe_fd, infile_fd, outfile_fd);
// 	close(pipe_fd[0]);
// 	waitpid(p2, &status, 0);
// 	return ((status >> 8) & 0xFF);
// }
